  ...
  return (
    <div>
      ...
      <p>
        ...
        The application is structured into micro services. In particular one of them serves the web client and the other one has a Restful API to receive the data sent by the weather station. This last one also stores the data into the database and serves them to the web client.<br/>
      ...
      <p>
        Since the main idea was to learn as mush as possible about AWS, I ended up with an implementation of the infrastructure that is much more complicated than the real needs of my application.<br/>
        The entire application runs on a private cloud of Amazon Web Services.<br/>
        After a general investigation of the various AWS components, I decided to use Elastic Container Service and Cloud Formation.<br/>
        I decided to use Cloud Formation after I had to do with the jungle of the interface that AWS presents to the users. In fact I definitely prefer to write all my configuration inside a template file and give it to Cloud Formation to be built.<br/>
        Another reason is because it allows me to edit the configuration of any part of the cloud simply by changing a couple of lines of that file.<br/>
        ...
        Furthermore every time that a task doesn’t answer anymore, ECS gets in action to substitute it. This is because it’s set to have a fixed number of desired tasks always running.<br/>
      ...
      <p>
        ...
        Both codebases of the two micro services are on GitHub. For now the whole code is developed on one single branch (master) because the application is still small, it’s not public (other then the public DNS of the Load Balancer) and because I’m the only one working on it.<br/>
        ...
        Every time there’s a commit of something to GitHub, SemaphoreCI is configured to automatically start to execute the tests and, if they pass, to build  the front end. Finally, it creates the docker image of the application and pushes the image to AWS Elastic Container Registry. After that, it requests the update of the Cloud Formation stack.<br/>
        Cloud Formation updates the stack with the new indications from the template and keeps the old ones unchanged. In this case, only the url of the image has changed, so Cloud Formation will only update the tasks that are running the containers with this image.<br/>
      ...
      <p>
        ...
        This also allowed me to make public the data gathered from the weather station and to show to it through graphs on a webpage.<br/>
        My chosen technologies to realise the application have been ReactJs with Redux, npm as package manager and webpack as module bundler.<br/>
        ...
        For the front end I decided to create a Single Page Application. For this purpose webpack dynamically creates the index page from a template that I give to it. The server serves this html page with the initial assets (images and fonts) and a javascript bundle.<br/>
        ...
        So the application is usable also offline only when browsing with Chrome.<br/>
